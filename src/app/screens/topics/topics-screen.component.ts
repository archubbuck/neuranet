...
@Component({
  ...
  template: `
      ...
      @if (tab() === 'records') {
        ...
        <div class="toolbar">
          ...
          <div class="cat-filter-wrap">
            ...
            <app-popover
              [open]="catFilterOpen()"
              (closed)="catFilterOpen.set(false)"
              style="top: 40px; left: 0; width: 200px; padding: 6px;"
            >
              <button
                class="cf-option"
                [class.cf-all]="categoryFilter() === ''"
                type="button"
              ...
              >
                <app-icon
                  name="layers"
                  [size]="13"
                  [color]="categoryFilter() === '' ? '#FBBF24' : '#475569'"
                />
              ...
              @for (c of store.clusters(); track c.id) {
                <button
                  class="cf-option"
                  [class.cf-active]="c.id === categoryFilter()"
                  type="button"
          ...
          <span class="toolbar-count"
            >{{ visibleRows().length }} of {{ rows().length }} topics</span
          >
        </div>
            ...
            <div class="thead">
              ...
              <div class="th-cell th-label">
                <button
                  class="th th-sort"
                  type="button"
                  (click)="toggleColumnSort('title', $event)"
                >
              ...
              <div class="th-cell th-num">
                <button
                  class="th th-sort"
                  type="button"
                  (click)="toggleColumnSort('linkCount', $event)"
                >
              ...
              <div class="th-cell th-num">
                <button
                  class="th th-sort"
                  type="button"
                  (click)="toggleColumnSort('sentiment', $event)"
                >
              ...
              <div class="tr" [class.sel]="on">
                ...
                <div class="td td-main">
                  <span
                    ...
                    [style.boxShadow]="'0 0 7px ' + r.color + '90'"
                  ></span>
                  @if (editingId() === r.id) {
                    <input
                      class="editor"
                ...
                <div class="td td-cat">
                  <button
                    ...
                    [style.color]="r.color"
                    (click)="toggleCategoryPicker(r.id)"
                  >
                    <span
                      class="cat-dot"
                      [style.background]="r.color"
                      [style.boxShadow]="'0 0 5px ' + r.color + '90'"
                    ></span>
                    {{ catLabel(r.cluster) }}
                    ...
                    <div class="cat-dropdown">
                      @for (c of store.clusters(); track c.id) {
                        <button
                          class="cat-option"
                          type="button"
                          [class.active]="c.id === r.cluster"
                          (click)="reassignOne(r, c.id)"
                        >
                ...
                <div
                  class="td td-num"
                  [class.sent-neg]="isNeg(r.sentiment)"
                  [class.sent-pos]="isPos(r.sentiment)"
                >
                  {{ fmtSentiment(r.sentiment) }}
                </div>
                  ...
                  <div class="menu-wrap">
                    ...
                    >
                      @if (rowMenuReassign() === r.id) {
                        ...
                        @for (c of store.clusters(); track c.id) {
                          <button
                            class="menu-reassign-item"
                            type="button"
                            (click)="
                              reassignOne(r, c.id); rowMenuId.set(null); rowMenuReassign.set(null)
                            "
                          >
                            <span class="dot-sm" [style.background]="c.color"></span>
                            <span>{{ c.label }}</span>
          ...
          <div class="bulk-bar">
            <div class="bulk-left">
              ...
              <span class="bulk-label">{{
                selectedIds().size === 1 ? 'topic selected' : 'topics selected'
              }}</span>
            </div>
            <button class="btn-secondary" type="button" (click)="startBulkReassign()">
              Reassign
            </button>
            <button
              class="btn-secondary"
            ...
            <button class="btn-secondary" type="button" (click)="startNewCatFromSelection()">
              New Category
            </button>
        ...
        @if (rows().length === 0) {
          ...
          <div class="analytics-scroll">
            ...
            <div class="stat-strip">
              ...
              <app-stat-card
                [value]="fmtSentiment(avgSentiment())"
                label="Avg sentiment"
    ...
    <app-modal
      ...
      [title]="'Reassign ' + selectedIds().size + ' topic(s)'"
    ...
    >
      ...
      <div footer>
        <button class="btn-ghost" type="button" (click)="bulkReassignOpen.set(false)">
          Cancel
        </button>
      </div>
    </app-modal>
    ...
    <app-modal
      [open]="creating()"
      title="New Topic"
      subtitle="Add a new node to the graph."
      (closed)="cancelCreate()"
    >
      <div class="field">
    ...
    <app-modal
      [open]="newCatOpen()"
      title="New Category from Selection"
    ...
    >
      <div class="field">
        ...
        <input
          ...
          type="text"
          [ngModel]="newCatLabel()"
          (ngModelChange)="newCatLabel.set($event)"
          placeholder="e.g. Machine Learning"
          (keydown.enter)="commitNewCatFromSelection()"
        />
      </div>
      <div footer>
        ...
        <button
          ...
          (click)="commitNewCatFromSelection()"
        >
          Create &amp; Move
        </button>
      </div>
...
export class TopicsScreenComponent {
  ...
  protected async onDeleteOne(id: string): Promise<void> {
    ...
    if (!confirm(`Delete topic "${node?.title ?? id}"? This will also drop all its connections.`))
      return;
  ...
  protected async onBulkMerge(): Promise<void> {
    ...
    if (!confirm(`Merge ${sourceSlugs.length} topic(s) into "${targetNode?.title ?? targetSlug}"?`))
      return;
  ...
  protected async onBulkDelete(): Promise<void> {
    const slugs = [...this.selectedIds()];
    if (!confirm(`Delete ${slugs.length} topic(s)? These will also be removed from the graph.`))
      return;
  ...
  protected readonly sentimentBands = computed(() => {
    ...
    for (const r of rows) {
      for (const b of bands) {
        if (r.sentiment! >= b.min && r.sentiment! < b.max) {
          b.count += 1;
          break;
        }
      }
