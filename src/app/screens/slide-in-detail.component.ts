		...
		<aside class="panel" [class.open]="open()">
			@if (node(); as n) {
				<header>
					...
					<button type="button" class="icon-btn" (click)="store.setDetailView('full')" title="Expand to full view">
	...
	styles: [
		`
			...
			.orb {
				...
				flex-shrink: 0;
				position: relative;
				background: radial-gradient(circle at 35% 35%, color-mix(in srgb, var(--accent) 80%, transparent), color-mix(in srgb, var(--accent) 53%, transparent));
				box-shadow: 0 0 20px color-mix(in srgb, var(--accent) 31%, transparent),
