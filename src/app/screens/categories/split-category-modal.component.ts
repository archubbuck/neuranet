  ...
  template: `
    ...
    >
        ...
        <div class="field">
          ...
          <div class="swatch-wrap">
            <button
              ...
              (click)="colorOpen.set(!colorOpen())"
            ></button>
            <app-popover
              [open]="colorOpen()"
              style="top: 44px; left: 0; padding: 12px; width: max-content;"
              (closed)="colorOpen.set(false)"
            >
              ...
              <div class="color-grid">
                @for (col of palette(); track col) {
                  <button
                    class="color-swatch"
                    type="button"
                    [attr.aria-label]="'Pick color ' + col"
                    [style.background]="col"
                    [class.picked]="col === color()"
                    (click)="color.set(col); colorOpen.set(false)"
                  ></button>
                }
              </div>
            </app-popover>
      ...
      <div class="split-node-list">
        @for (n of members(); track n.id) {
          ...
          <button class="split-node" type="button" [class.picked]="on" (click)="togglePick(n.id)">
            ...
            <span
              class="split-node-dot"
              [style.background]="on ? color() : (cat()?.color ?? '')"
              [style.boxShadow]="'0 0 6px ' + (on ? color() : (cat()?.color ?? '')) + '80'"
            ></span>
      ...
      <div footer>
        ...
        <button class="btn-primary" type="button" [disabled]="!canSplit()" (click)="submit()">
          Split {{ picked().size || '' }} node{{ picked().size === 1 ? '' : 's' }} out
        </button>
      </div>
    </app-modal>
  `,
