 ...
 *     rejects a write
 *
 * Persistence: a tiny slice of state (`screen`, `collapsed`) is mirrored
...
export class AppStore {
	...
	readonly clusterStats = computed(() => {
		...
		const stats = new Map<string, { count: number; docs: number; sentSum: number; sentCount: number }>();
	...
	readonly avgSentiment = computed(() => {
		...
		let count = 0;
		for (const n of this.nodes()) {
			if (n.sentiment != null) { sum += n.sentiment; count++; }
		}
	...
	private readonly _screen = signal<Screen>('network');
	...
	private hydrate(): void {
		try {
			...
			if (!raw) return;
			const parsed = JSON.parse(raw) as { screen?: Screen; collapsed?: boolean };
			if (parsed.screen) this._screen.set(parsed.screen);
	...
	async fetchSource(sourceId: number): Promise<void> {
		...
		try {
			...
			this._sources.update((list) =>
				list.map((s) => (s.id === sourceId ? source : s)),
			);
			this.updateJob(sourceId, { status: source.status, message: source.status_message ?? undefined });
			const [network, docs] = await Promise.all([
				this.api.getNetwork(),
				this.api.listDocs(),
			]);
			this._network.set(network);
	...
	async createCluster(label: string, color?: string): Promise<void> {
		...
		this._network.update((net) => ({
			...net,
			derivedClusters: [...net.derivedClusters, { id: created.id, label: created.label, color: created.color }],
		}));
	}
	...
	async deleteCluster(slug: string): Promise<void> {
		...
		if (this._selectedNodeId() != null && !this.nodes().some((n) => n.id === this._selectedNodeId())) {
	...
	async renameNode(slug: string, label: string): Promise<void> {
		...
		this._network.update((net) => ({
			...
			derivedNodes: net.derivedNodes.map((n) =>
				n.id === slug ? { ...n, label: updated.label, desc: updated.desc, cluster: updated.cluster } : n,
			),
		}));
	}
	...
	async bulkDeleteNodes(slugs: string[]): Promise<void> {
		...
		this._network.update((net) => ({
			...
			derivedEdges: net.derivedEdges.filter((e) => !slugs.includes(e.source) && !slugs.includes(e.target)),
		}));
