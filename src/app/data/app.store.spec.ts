...
describe('AppStore', () => {
	...
	async function seedTwoClusters(): Promise<void> {
		...
		http.expectOne('/api/network').flush({
			...
			derivedNodes: [
				{ id: 'n1', label: 'One', desc: null, cluster: 'c1', r: 10, importance: 5, depth: 0, isCentral: false, degree: 1 },
				{ id: 'n2', label: 'Two', desc: null, cluster: 'c1', r: 10, importance: 5, depth: 0, isCentral: false, degree: 1 },
			],
	...
	function flushLoadAll(): void {
		http.expectOne('/api/sources').flush([]);
		http.expectOne('/api/network').flush({ derivedClusters: [], derivedNodes: [], derivedEdges: [] });
		http.expectOne('/api/docs').flush([]);
	}
